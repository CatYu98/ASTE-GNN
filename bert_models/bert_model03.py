...
#         relevant_sentence_out0 = self.bert_model(relevant_sentences_presentation.view(-1, rele_sen_len))[0]
#         relevant_sentence_out0 = relevant_sentence_out0.view(batch_size, -1,768)
...
#         '''graph3'''
#         formal_global_features1 = torch.cat([text_out, relevant_sentence_out0], 1)
#         attention_feature1 = formal_global_features1
#         attention1 = torch.matmul(attention_feature1, attention_feature1.permute(0, 2, 1))
#         attention1 = F.softmax(attention1, -1)
#         norm_global_adj1 = preprocess_adj(attention1)
#         global_text_out1 = self.gcn(norm_global_adj1, formal_global_features1)[:, :sentence_len, :]
#         # concatenate
#         unified_text = torch.cat([text_out.float(), global_text_out1.float()], -1)
#         # AE and OE scores
...
class BERT_GCN03(nn.Module):
    def __init__(self, opt, pretrained_model='bert-base-uncased', freeze_bert = False):
        super(BERT_GCN03, self).__init__()
        # 'roberta-base', 'roberta-large', 'bert-base-uncased', 'bert-large-uncased'
        ...
        self.gcn2 = GCNforFeature_1(dim_dic[pretrained_model], 300, 0.5)
        self.gcn3 = GCNforFeature_2(dim_dic[pretrained_model], 300, 300, 0.5)
        self.gcn4 = GCNforFeature_2(dim_dic[pretrained_model], 300, 300, 0.5)
        self.gcn5 = GCNforFeature_2(dim_dic[pretrained_model], 300, 300, 0.5)

        self.aspect_opinion_classifier = SequenceLabelForAO((dim_dic[pretrained_model]+300+300+300)*2, 3, 0.5)
        self.triple_classifier = SequenceLabelForTriple((dim_dic[pretrained_model]+300+300+300)*2, 3, 0.5)
    ...
    def forward(self, inputs, mask):
        ...
        relevant_sentence_out3 = self.bert_model(relevant_sentences_presentation.view(-1, rele_sen_len))[0].view(batch_size, rele_sen_num*rele_sen_len,-1)
        # get local and global 
        attention_feature_local, attention_feature_global = text_out, relevant_sentence_out
        attention_feature_global3 = relevant_sentence_out3
        ...
        attention_global3 = torch.matmul(attention_feature_local, attention_feature_global3.permute(0, 2, 1))
        attention_local, attention_global = F.softmax(attention_local, -1), F.softmax(attention_global, -1)
        attention_global3 = F.softmax(attention_global3, -1)
        formal_attention_local, formal_attention_global = generate_formal_adj(attention_local), generate_formal_adj(attention_global)
        formal_attention_global3 = generate_formal_adj(attention_global3)
        norm_local_adj, norm_global_adj = preprocess_adj(formal_attention_local), preprocess_adj(formal_attention_global)
        norm_global_adj3 = preprocess_adj(formal_attention_global3)
        # get features
        formal_global_features3 = torch.cat([text_out, relevant_sentence_out3], 1)
        formal_global_features = torch.cat([text_out, relevant_sentence_out], 1)
        ...
        if self.opt.gcn_layers_in_graph0 == 1:
            ...
            global_text_out3 = self.gcn2(norm_global_adj3, formal_global_features3)[:, :sentence_len, :]
            ...
            global_text_out3 = self.gcn5(norm_global_adj3, formal_global_features3)[:, :sentence_len, :]
        # concatenate
        unified_text = torch.cat([text_out.float(), local_text_out.float(), global_text_out.float(), global_text_out3.float()], -1)
        # pair generation
        pair_text = self.pairgeneration(unified_text)
        # AE and OE scores
