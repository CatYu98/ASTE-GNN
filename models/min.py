...
class MIN(nn.Module):
    ...
    def forward(self, inputs, mask):
        ...
        # GCN with local graph and global graph
        local_text_out = self.gcn(text_out, local_adj_pmi) # 32, 13, 300
        global_text_out = self.gcn_(sentence_text_out1, global_adj) # 32, 13, 300
        ...
        # print(pair_text.shape)

        # fc        
        pair_probs = self.fc_pair(pair_text.float().contiguous().view(-1, pair_text.shape[-1]))
        ...
        # pair_probs = opinion_probs.contiguous().view(text_indices.shape[0], -1, pair_text.shape[-1])
