...
class TS5(nn.Module):
    def __init__(self, embedding_matrix, opt):
        ...
        self.aspect_opinion_sentiment_classifier = SequenceLabelForAOS(300, 3, 0.5)

        self.sentiment_classifier = MultiNonLinearClassifier(300, 4, 0.5)

        self.pair_classifier = MultiNonLinearClassifier(600, 3, 0.5)
        self.pair_sentiment_classifier =  MultiNonLinearClassifier(600, 4, 0.5)
        
        self.triple_classifier = SequenceLabelForTriple(1800, 3, 0.5)
        self.grid_classifier = SequenceLabelForGrid(600, 3, 0.5)
    def forward(self, inputs, mask):
        ...
        # unified_text = torch.cat([text_out.float(), global_text_out2.float(), local_text_out.float()], -1)
        unified_text = torch.cat([text_out.float(), global_text_out2.float()], -1)
        # unified_text = global_text_out2.float()
        # pair generation
        pair_text = self.pairgeneration(unified_text)
        # AE and OE scores
