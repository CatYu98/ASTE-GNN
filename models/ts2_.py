# # -*- coding: utf-8 -*-
# # we create this model to test the effectiveness global_graph0 + global_graph2 
...
# class TS2_(nn.Module):
#     def __init__(self, embedding_matrix, opt):
#         super(TS2_, self).__init__()
#         self.opt = opt
...
#         self.gcn0 = GCN(300, 150, 3, 0.3)
#         self.gcn1 = GCN(300, 150, 3, 0.3)
#         self.gcn2 = GCN(300, 150, 3, 0.3)
#         self.sigmoid = nn.Sigmoid()
#         self.text_embed_dropout = nn.Dropout(0.3)
#         self.soft = nn.Softmax(dim=-1)
#         self.pairgeneration = PairGeneration0(450)

#         self.fc_aspect_h = nn.Linear(450, 150)
#         self.fc_opinion_h = nn.Linear(450, 150)
#         self.fc_sentiment_h = nn.Linear(450, 150)
#         self.fc_pair_h = nn.Linear(900, 450)
#         self.fc_pair_sentiment_h = nn.Linear(900, 450)

#         self.fc_aspect = nn.Linear(150, 3)
#         self.fc_opinion = nn.Linear(150, 3)
#         self.fc_sentiment = nn.Linear(150, 4)
#         self.fc_pair = nn.Linear(450, 3)
#         self.fc_pair_sentiment = nn.Linear(450, 4)

#         self.fc_s0 = nn.Linear(600, 300)
#         self.fc_r_s = nn.Linear(600, 300)
#         self.fc_r_s_w = nn.Linear(600, 300)
#         self.fc_unified_sen = nn.Linear(450, 300)
...
#         sentence_text_out, (sentence_text_out1, b_) = self.lstm_(sentence_embedding, torch.where(sentence_text_len <= 0, ones, sentence_text_len))
...
#         # get a unified relevant sentence features
#         unified_rele_sentence_features = self.fc_unified_sen(torch.cat([sentence_text_out1_narrow, global_text_out1], -1))
#         '''graph 0'''
#         # process formal features to match the formal adj
#         formal_global_features = torch.cat([text_out_narrow, unified_rele_sentence_features], 1)
...
#         # unified features
#         unified_text = torch.cat([text_out_narrow.float(), global_text_out2.float()], -1)
#         # pair generation
...
class TS2_(nn.Module):
    def __init__(self, embedding_matrix, opt):
        ...
        self.fc_aspect_h = nn.Linear(450, 300)
        self.fc_opinion_h = nn.Linear(450, 300)
        self.fc_sentiment_h = nn.Linear(450, 300)
        self.fc_pair_h = nn.Linear(900, 600)
        self.fc_pair_sentiment_h = nn.Linear(900, 600)
