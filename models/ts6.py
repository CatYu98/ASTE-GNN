...
#     def __init__(self, in_dim, out_dim, p, acti=True):
#         super(GraphSage, self).__init__()
#         self.linear = nn.Linear(in_dim*2, in_dim, bias=False) # bias = False is also ok.
#         if acti:
#             # self.acti = nn.ReLU(inplace=True)
#             self.acti = nn.PReLU()
#         else:
...
#         self.gs1 = GraphSage(600, 300, 0.5)
#         self.gs2 = GraphSage(600, 300, 0.5)
...
#         formal_global_features = torch.cat([text_out, sentence_text_out2], 1)
#         # attention
...
#         # Graphsage
#         # global_text_out1 = self.gs1(norm_global_adj, formal_global_features)
#         # global_text_out2 = self.gs2(norm_global_adj, global_text_out1)
#         # unified features
#         unified_text = torch.cat([text_out.float(), global_text_out.float()], -1)
#         # unified_text = global_text_out1[:, :sentence_len, :]
#         # unified_text = text_out
#         # pair generation
...
#         # pair scores 
#         pair_probs_, triple_probs_ = self.triple_classifier(pair_text.float()*pair_mask_grid)
...
class TS6(nn.Module):
    def __init__(self, embedding_matrix, opt):
        ...
        self.aspect_opinion_classifier = SequenceLabelForAO(3000, 3, 0.5)
        
        self.triple_classifier = SequenceLabelForTriple(3000, 3, 0.5)
    def forward(self, inputs, mask):
        ...
        # unified features
        unified_text = torch.cat([text_out.float(), local_text_out.float(), global_text_out.float(), global_text_out0.float()], -1)
        # pair generation
        pair_text = self.pairgeneration(unified_text)
        # AE and OE scores (BIO tagging)
