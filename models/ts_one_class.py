...
class TS_one_class(nn.Module):
    def __init__(self, embedding_matrix, opt):
        super(TS_one_class, self).__init__()
        self.opt = opt
    ...
    def forward(self, inputs, mask):
        ...
        # 改两个二分类为一个三分类
        aspect_opinion_probs, _ = self.aspect_opinion_classifier(pair_text.float())
        aspect_probs, opinion_probs = aspect_opinion_probs[:,:,1], aspect_opinion_probs[:,:,2]
        ...
        return aspect_opinion_probs, pair_probs, triple_probs, [selected_aspect.indices, selected_opinion.indices]