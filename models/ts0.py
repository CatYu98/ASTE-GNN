...
'''class SequenceLabelForAO(nn.Module):
    def __init__(self, hidden_size, tag_size, dropout_rate):
        super(SequenceLabelForAO, self).__init__()
        self.tag_size = tag_size
        self.linear1 = nn.Linear(hidden_size, int(hidden_size / 4))
    ...
    def forward(self, input_features):
        ...
        Args:
            input_features: (bs, seq_len, h)
        """
        sentence_len = input_features.shape[1]
        batch_size = input_features.shape[0]

        features_sub = self.linear1(input_features)
        features_sub = nn.ReLU()(features_sub)
        features_sub = self.dropout(features_sub)

        features_obj = self.linear2(input_features)
        features_obj = nn.ReLU()(features_obj)
        features_obj = self.dropout(features_obj)

        attention_s2o = F.softmax(torch.matmul(features_sub.reshape([batch_size, sentence_len*sentence_len, -1]), features_obj.reshape([batch_size, sentence_len*sentence_len, -1]).permute(0,2,1)), dim = -1)
        attention_o2s = F.softmax(torch.matmul(features_obj.reshape([batch_size, sentence_len*sentence_len, -1]), features_sub.reshape([batch_size, sentence_len*sentence_len, -1]).permute(0,2,1)), dim = -1)

        features_sub_ = torch.matmul(attention_s2o, features_obj.reshape([batch_size, sentence_len*sentence_len, -1])).reshape([batch_size, sentence_len, sentence_len, -1])
        features_obj_ = torch.matmul(attention_o2s, features_sub.reshape([batch_size, sentence_len*sentence_len, -1])).reshape([batch_size, sentence_len, sentence_len, -1])

        features_sub_tmp = torch.cat([features_sub, features_sub_], dim=-1)
        features_obj_tmp = torch.cat([features_obj, features_obj_], dim=-1)

        sub_output = self.hidden2tag_sub(features_sub_tmp)
        obj_output = self.hidden2tag_obj(features_obj_tmp)
...
class TS0(nn.Module):
    ...
    def forward(self, inputs, mask):
        # input
        ''' 加_是限制来自同一个句子，span是指读取了span之内的所有index，start_end是只存了span的开始位置和结束位置 '''
        ...
        '''这一部分是使用了span的所有表示，而非只用start和end '''
        ...
        '''不限制来自同一个句子，去重或者不去重两个版本，通过修改data_utils来改变'''
        # --------------------------------------------------------------
        ...
        # 直接concat
        '''aspect_start_end_out = torch.cat([aspect_start_end_out[:,0, :], aspect_start_end_out[:,1, :]], dim=-1).unsqueeze(0).repeat(batch_size, 1, 1)
        opinion_start_end_out = torch.cat([opinion_start_end_out[:,0, :] + opinion_start_end_out[:,1, :], dim=-1).unsqueeze(0).repeat(batch_size, 1, 1)'''
        # --------------------------------------------------------------
        
        '''限制来自同一个句子，index相互对应'''
        # --------------------------------------------------------------
        # train_start_end_embedding, 加了下划线，表示限制来自同一个句子
        ...
        # start end concat **1
        aspect_start_end_out_ = torch.cat([aspect_start_end_out_[:,0, :], aspect_start_end_out_[:,1, :]], dim=-1).unsqueeze(0).repeat(batch_size, 1, 1)
        opinion_start_end_out_ = torch.cat([opinion_start_end_out_[:,0, :], opinion_start_end_out_[:,1, :]], dim=-1).unsqueeze(0).repeat(batch_size, 1, 1)
        # --------------------------------------------------------------
        ...
        # pair generation, 这里要不要乘以一个距离有关的向量，或者concat一个与距离有关的可训练的向量。
        pair_text = self.pairgeneration(text_out)
        # AE and OE scores (SPAN tagging)，这个地方可以加一个pair attention，以及是否需要提前mask上三角。
        aspect_probs, opinion_probs = self.aspect_opinion_classifier(pair_text.float())

        '''这里是得到候选的aspect和opinion的下标'''
        # --------------------------------------------------------------
        # mask只留下上三角的概率值，下三角表示的span没有意义。
        ...
        # 先选每一行概率值最大的，其实可以理解为先选最可能是aspect和opinion的end的坐标。
        ...
        # 其次根据values选择一列里面概率值最大的，可以理解为选最可能是aspect和opinion的start的坐标。
        ...
        # 要选择_k_k（最大的aspect和opinion的start对应的最大的aspect和opinion的end。）
        aspect_indices_k_shrink = torch.gather(aspect_likelihood_k.indices.squeeze(-1), 1, aspect_likelihood_k_k.indices).unsqueeze(-1)
        ...
        # 分别得到start和end的表示然后拼起来，得到最后的span的表示 
        # **2 必须保证和**1得到表示的方法相同，在同一个空间内
        ...
        # 分别得到start和end的表示，然后加起来
        aspect_start_end_representation = aspect_start_representation + aspect_end_representation
        opinion_start_end_representation = opinion_start_representation + opinion_end_representation
        # --------------------------------------------------------------
        
        ''' 注意计算相似度的tensor要保持在同一个向量空间内 '''
        ''' Alternative Solution1 不限制relevant spans来自同一个句子'''
        # --------------------------------------------------------------
        ...
        grid_representation = self.gridgeneration(new_aspect_representation, new_opinion_representation)
            ...
            # 调换的话要从上一个计算similarity的section就开始调换
            # 也可以对同一个text_out进行两遍填充，然后再得到grid_representation
        text_out_aspect = text_out.scatter(dim = 1, index = aspect_indices[:,:,0].unsqueeze(-1).repeat(1,1,600), src = new_aspect_representation)
        text_out_opinion = text_out.scatter(dim = 1, index = opinion_indices[:,:,0].unsqueeze(-1).repeat(1,1,600), src = new_opinion_representation)
        new_grid_representation = self.gridgeneration(text_out_aspect, text_out_opinion)
        # 是否要对new_grid_representation做一个self attention
        # --------------------------------------------------------------

        ''' Alternative Solution2 限制来自同一个句子的做法 '''
        '''现在只改了限制来自同一个句子的做法，就是span的表示不再使用start，而是start和end拼起来 '''
        # --------------------------------------------------------------
        ...
        aspect_similarity_same = F.softmax(torch.matmul(aspect_span_representation, aspect_start_end_out_.permute(0,2,1)), dim = -1)
        # opinion_similarity_same = F.softmax(torch.matmul(opinion_start_representation, opinion_start_end_out_.permute(0,2,1)), dim = -1)
        opinion_similarity_same = F.softmax(torch.matmul(opinion_span_representation, opinion_start_end_out_.permute(0,2,1)), dim = -1)
        # 这里计算union的方式可以修改一下，思考一下
        union_similarity_same = aspect_similarity_same + opinion_similarity_same
        ...
        selected_aspect_representation_same = torch.gather(aspect_start_end_out_, 1 , selected_aspect_same.indices.reshape(batch_size, -1).unsqueeze(-1).repeat(1,1,aspect_start_end_out_.shape[-1])).reshape(batch_size, top_k_num, rele_term_num_same_sentence, -1)
        selected_opinion_representation_same = torch.gather(opinion_start_end_out_, 1 , selected_opinion_same.indices.reshape(batch_size, -1).unsqueeze(-1).repeat(1,1,aspect_start_end_out_.shape[-1])).reshape(batch_size, top_k_num, rele_term_num_same_sentence, -1)
        # 临时添加的，这里往前是按照cancat计算span的，这里开始变回去，相加，为了方便下面的span分类，后边这里可以改
        selected_aspect_representation_same = selected_aspect_representation_same[:,:,:,:600] + selected_aspect_representation_same[:,:,:,600:]
        selected_opinion_representation_same = selected_opinion_representation_same[:,:,:,:600] + selected_opinion_representation_same[:,:,:,600:]
        # 无交叉 这里权重可以改成小权重
        selected_opinion_representation_same = (selected_opinion_representation_same * F.softmax(selected_opinion_same.values, dim=-1).unsqueeze(-1).repeat(1,1,1,selected_opinion_representation_same.shape[-1]))
        selected_aspect_representation_same = (selected_aspect_representation_same * F.softmax(selected_aspect_same.values, dim=-1).unsqueeze(-1).repeat(1,1,1,selected_aspect_representation_same.shape[-1]))
        #0 new_aspect_representation_same = aspect_start_representation + selected_aspect_representation_same.sum(-2)
        new_aspect_representation_same = aspect_span_representation[:,:,:600] + aspect_span_representation[:,:,600:] + selected_aspect_representation_same.sum(-2)
        #0 new_opinion_representation_same =  opinion_start_representation + selected_opinion_representation_same.sum(-2)
        new_opinion_representation_same =  opinion_span_representation[:,:,:600] + opinion_span_representation[:,:,600:] + selected_opinion_representation_same.sum(-2)
        shrink_grid_representation_same = self.gridgeneration(new_aspect_representation_same, new_opinion_representation_same)
        text_out_aspect_same = text_out.scatter(dim = 1, index = aspect_indices[:,:,0].unsqueeze(-1).repeat(1,1,600), src = new_aspect_representation_same)
        text_out_opinion_same = text_out.scatter(dim = 1, index = opinion_indices[:,:,0].unsqueeze(-1).repeat(1,1,600), src = new_opinion_representation_same)
        new_grid_representation_same = self.gridgeneration(text_out_aspect_same, text_out_opinion_same)
        # --------------------------------------------------------------

        '''最开始的那种得到grid representation的方法，很快被否定了'''
        # --------------------------------------------------------------
        ...
        # classifier ??
        # 是否使用检索到的span？先找到candidate，gather抽取出他们的表示，分类，再scatter？ 
        # 不限制来自同一个句子，把top k span的表示加权以后加上去？
        # 限制来自同一个句子，把top k span的表示加权加上去？
        if self.opt.mode == 2:
        ...
        if self.opt.mode == 1:
            pair_probs_, triple_probs_ = self.triple_classifier(pair_text.float())
        # 先算概率再scatter
        '''pair_probs_, triple_probs_ = self.triple_classifier(shrink_grid_representation_same)
        indice_for_scatter = aspect_indices[:,:,0].unsqueeze(-1).repeat(1,1,top_k_num)*sentence_len + opinion_indices[:,:,0].unsqueeze(-1).repeat(1,1,top_k_num).permute(0,2,1)
        indice_for_scatter_pair = indice_for_scatter.reshape(batch_size, -1).unsqueeze(-1).repeat(1,1,3)
        indice_for_scatter_sentiment = indice_for_scatter.reshape(batch_size, -1).unsqueeze(-1).repeat(1,1,4)
        ...
        triple_probs = torch.zeros([batch_size, sentence_len*sentence_len, 4]).cuda().scatter(dim=1, index=indice_for_scatter_sentiment, src=triple_probs_.reshape(batch_size, -1, 4))'''
        # pdb.set_trace()
        ...
        pair_probs = pair_probs_.contiguous().view(-1, 3)
        triple_probs = triple_probs_.contiguous().view(-1, 4)
        return aspect_probs, opinion_probs, pair_probs, triple_probs, [selected_aspect.indices, selected_opinion.indices]