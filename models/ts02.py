...
#         self.fc_sentiment_h = nn.Linear(900, 300)
#         self.fc_pair_h = nn.Linear(1800, 600)
#         self.fc_pair_sentiment_h = nn.Linear(1800, 600)

#         self.fc_aspect = nn.Linear(300, 3)
#         self.fc_opinion = nn.Linear(300, 3)
#         self.fc_sentiment = nn.Linear(300, 4)
#         self.fc_pair = nn.Linear(600, 3)
#         self.fc_pair_sentiment = nn.Linear(600, 4)
...
#         # global_text_out = self.gcn1(norm_global_adj, global_text_out)[:, :sentence_len, :]
#         # unified features
...
#         # aspect_probs = self.fc_aspect(global_text_out.float()).contiguous().view(-1, 3)
#         # opinion_probs = self.fc_opinion(global_text_out.float()).contiguous().view(-1, 3)
#         # sentiment_probs = self.fc_sentiment(global_text_out.float()).contiguous().view(-1, 4)
...
#         # pair scores     
#         pair_probs_ = self.fc_pair_h(pair_text.float()*pair_mask_grid)
#         pair_probs = self.fc_pair(pair_probs_).contiguous().view(-1, 3)
...
#         # # pair scores     
#         # pair_probs = self.fc_pair(pair_text.float()*pair_mask_grid).contiguous().view(-1, 3)
#         # # pair sentiment scores     
...
class TS0(nn.Module):
    def __init__(self, embedding_matrix, opt):
        ...
        self.aspect_classifier = MultiNonLinearClassifier(900, 3, 0.5)
        self.opinion_classifier = MultiNonLinearClassifier(900, 3, 0.5)
        self.aspect_opinion_classifier = SequenceLabelForAO(1200, 3, 0.5)
        self.customize_aspect_opinion_classifier = CustomizeSequenceLabelForAO(900, 3, 0.5)
        self.aspect_opinion_sentiment_classifier = SequenceLabelForAOS(900, 3, 0.5)

        self.sentiment_classifier = MultiNonLinearClassifier(1200, 4, 0.5)

        self.pair_classifier = MultiNonLinearClassifier(1800, 3, 0.5)
        self.pair_sentiment_classifier =  MultiNonLinearClassifier(1800, 4, 0.5)
        
        self.triple_classifier = SequenceLabelForTriple(2400, 3, 0.5)
        self.grid_classifier = SequenceLabelForGrid(1200, 3, 0.5)
    def forward(self, inputs, mask):
        ...
        formal_global_features = torch.cat([text_out, sentence_text_out1], 1)
        # GCN with local global graph
        # norm_global_adj = norm_global_adj - torch.eye(norm_global_adj.shape[-1]).cuda()
        ...
        unified_text = torch.cat([text_out.float(), global_text_out_tem.float()], -1)
        ...
        pair_mask_grid = torch.unsqueeze(pair_mask,-1).repeat(1,1,1,pair_text.shape[-1])
        # pair scores 
        if self.opt.use_aspect_opinion_sequence_mask == 1:     
            ...
            if self.opt.emb_for_ps == 'private_single':
                ...
                pair_probs = pair_probs_.contiguous().view(-1, 3)
                pair_sentiment_probs = pair_sentiment_probs_.contiguous().view(-1, 3)


        # return F.log_softmax(aspect_probs, dim=-1), F.log_softmax(opinion_probs, dim=-1), F.log_softmax(sentiment_probs, dim=-1), F.log_softmax(pair_probs, dim=-1), F.log_softmax(pair_sentiment_probs, dim=-1)
        return F.log_softmax(aspect_probs, dim=-1), F.log_softmax(opinion_probs, dim=-1), F.log_softmax(sentiment_probs, dim=-1), F.log_softmax(aspect_grid_label, dim=-1), F.log_softmax(opinion_grid_label, dim=-1), F.log_softmax(pair_probs, dim=-1), F.log_softmax(pair_sentiment_probs, dim=-1)